...
class Viewer extends SimulationViewer {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 'g':
					if(_selectedGroupIdx ==  (_groups.size()-1)) {
						...
						_selectedGroupIdx++;
						String _selectedG = _groups.get(_selectedGroupIdx).getId();
						_selectedGroup = _selectedG;
					}
					repaint();
					break;
				default:
	...
	private void showHelp(Graphics2D g) {
        ...
        text =  "l: move right, j: move left, i: move up, m: move down: k: reset";
		g.drawString(text, 10, 40);
        text = "g: show next group";
        g.drawString(text, 10, 60);
        text = "Scaling ratio: " + _scale;
        g.drawString(text, 10, 80);
        text = "Selected Group: " + _selectedGroup;
	...
	private void drawBodies(Graphics2D g) {
		...
		for (Body b : _bodies) {
		    if (isVisible(b)) {
				...
				g.drawString(b.getId(), x-10, y-10);
				
		    	if(_showVectors) {
		    		Vector2D velocity = b.getVelocity().direction().scale(20);
	                Vector2D force = b.getForce().direction().scale(20);
		    		int velX = x+(int) velocity.getX();
		            int velY = y-(int) velocity.getY();
		           drawLineWithArrow(g, x, y, velX, velY,10, 10,Color.green,Color.green);
		            
		            int forceX = x +(int) force.getX();
	                int forceY = y-(int)force.getY();
	                drawLineWithArrow(g, x, y, forceX, forceY,10,10,Color.red,Color.red);
